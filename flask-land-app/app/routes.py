...
def filtrar_propiedades():
    ...
    departamento_id_str = request.args.get('departamento', '')
    ciudad_id_str = request.args.get('ciudad', '')
    ...
    moneda_filtro_id_str = request.args.get('moneda_filtro_id', '')

    # Convertir a tipos adecuados y preparar para la plantilla
    precio_min_val = int(precio_min_str) if precio_min_str.isdigit() else None
    precio_max_val = int(precio_max_str) if precio_max_str.isdigit() else None
    moneda_filtro_id_selected = moneda_filtro_id_str if moneda_filtro_id_str.isdigit() else None
    ...
    ciudades_query = "SELECT id, nombre FROM ciudad"
    if departamento_id_str.isdigit():
        ciudades_query += f" WHERE departamento_id = {departamento_id_str}"
    ciudades = db.session.execute(text(ciudades_query)).mappings().fetchall()
    ...
    elif (precio_min_val is not None or precio_max_val is not None): # Si no se seleccionó moneda, asumir Gs. para el input
        gs_moneda = next((m for m in monedas_cotizacion if m.tipo_moneda.upper() == 'GS.'), None) # Buscamos Gs.
        if gs_moneda and gs_moneda.valor_cotizacion == 1: # Asumimos que Gs tiene valor_cotizacion 1
             if precio_min_val is not None:
                precio_min_gs = precio_min_val
             if precio_max_val is not None:
                precio_max_gs = precio_max_val
        # Si no se encuentra Gs. o su cotización no es 1, el filtro de precio podría no aplicarse
        # o podrías forzar una moneda por defecto. Por ahora, si no hay Gs. con cot 1, no se filtra.
